...
`llm.rs` is a thin wrapper around an OpenAI-compatible `/v1/chat/completions` endpoint. It can attach a base64-encoded screenshot to the user message when `screen_vision` is enabled. Long for ai slop vision
...
`name` is the list identifier. It is used by `comment_lists.active` in config to filter which lists are loaded. If omitted, defaults to the filename without extension.

`triggers` is a list of substrings matched case-insensitively against the current window name or bash command. Any match is enough to include the list as a candidate. If triggers is empty, the list is treated as a generic fallback pool and can fire whenever no trigger-based list matches.

`animation` is the Clippy animation to play when a comment from this list fires. See the animations section below. Optional; if omitted, clippy-linux picks one based on context keywords in the matched string.

`comments` is the list of comment strings. One is picked at random each time the list fires. `%s` anywhere in the string is replaced with the raw context value (window name or command).
...
When `screen_vision` is true, clippy-linux captures a screenshot via the XDG screenshot portal, encodes it as base64 PNG, and attaches it to the user message. The file is deleted immediately after. Requires a model with vision support (gpt-4o works, gpt-4o-mini works, most local models do not).

On timeout or any request error, clippy-linux falls back to static comment lists silently.

And yes the prompt is ai generated, i aint writing this shit
